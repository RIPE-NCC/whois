...
public class NrtmConcurrencyTestIntegration extends AbstractNrtmIntegrationBase {
    ...
    private void loadSerials(int min, int max) {
        ...
        whoisTemplate.update("UPDATE last SET timestamp = ?", new Timestamp(testDateTimeProvider.getCurrentDateTime()).getValue());
        whoisTemplate.update("UPDATE history SET timestamp = ?", new Timestamp(testDateTimeProvider.getCurrentDateTime()).getValue());
    }

...
The result explained:
The program will create a file called sponsoringlirs that contains emails of the sponsoring lirs. The are unique, non-ordered and approx 5000.
The console output will be on this format:
<organisation>|<email>
The <organisation> is the organisation without an abuse contact.
The <email> is the first e-mail of <organisation> as stated in the whois database.
...
How are we getting the results?
We take all inet(6)nums with status assigned pi and all autnums from the ripe database and for all those who have no abuse contact we check with rsng that it has status ENDUSER_APPROVEDDOCS and that it's not legacy space.


Limitations?
* The inet(6)nums and autnums that has no org attribute are quietly disregarded
TBC
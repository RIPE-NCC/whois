...
class SyncUpdateResponseTest extends Specification {
    SyncUpdateResponse response = new SyncUpdateResponse(""""
...
""")
    ...
    def errorSection() {
      when:
        def errorSection = response.errorSection
      then:
        errorSection == """---
Delete FAILED: [mntner] DEL-MNT
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"""
    ...
    def successSection() {
      when:
        def successSection = response.successSection
      then:
        successSection == "---\nCreate SUCCEEDED: [mntner] MOD-MNT\n***Warning: Some warning\n\n---\nCreate SUCCEEDED: [mntner] MOD2-MNT\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    }
    ...
    def successes() {
      when:
        def successes = response.successes
      then:
        successes.size() == 2
    ...
    def test_suncUpdate_failed() {
        when:
        response.contents = "asdfasdf FAILED: \n asdad SUCCEEDED:"

        then:
        response.failed
    }

    def test_suncUpdate_success() {
        when:
        response.contents = "asdfasdf SUCCEEDED: "

        then:
        response.success
    }

    def test_suncUpdate_no_fail_no_success() {
        when:
        response.contents = "asdasd "

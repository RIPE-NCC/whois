...
import static net.ripe.db.whois.api.ApiKeysAuthServerDummy.BASIC_AUTH_SHORT_API_KEY;
...
public class WhoisRestApiKeyAuthTestIntegration extends AbstractHttpsIntegrationTest {
    ...
    public void create_mntner_only_data_parameter_with_apiKey_fails_invalid() {
        ...
        final String response = SecureRestTest.target(getSecurePort(), "whois/syncupdates/test")
                .request()
                .header(HttpHeaders.AUTHORIZATION, getBasicAuthHeader(BASIC_NON_EXISTING_API_KEY))
                .post(Entity.entity("DATA=" +  SyncUpdateUtils.encode(mntner),
                        MediaType.valueOf("application/x-www-form-urlencoded")), String.class);
    ...
    @Test
    public void create_object_with_short_apikey_then_401() {
        databaseHelper.updateObject(RpslObject.parse("" +
                "mntner:      OWNER-MNT\n" +
                "descr:       Owner Maintainer\n" +
                "admin-c:     TP1-TEST\n" +
        ...
        Response response = SecureRestTest.target(getSecurePort(), "whois/test/person")
                .request()
                .header(HttpHeaders.AUTHORIZATION, getBasicAuthHeader(BASIC_AUTH_SHORT_API_KEY))
                .post(Entity.entity(map(PAULETH_PALTHEN), MediaType.APPLICATION_XML), Response.class);
    ...
    public void create_mntner_only_data_parameter_with_apiKey_fails_wrong_audience() {
        ...
        assertThat(response, containsString("Create FAILED: [mntner] SSO-MNT"));
        assertThat(response, containsString("***Warning: [APIKEY] An error occurred while attempting to decode the Jwt:\n" +
    ...
    public void create_mntner_only_data_parameter_with_apiKey_fails_invalid_iss_claim(){
        ...
        assertThat(response, containsString("Create FAILED: [mntner] SSO-MNT"));
        assertThat(response, containsString("***Warning: [APIKEY] An error occurred while attempting to decode the Jwt: The\n" +
    ...
    public void create_mntner_only_data_parameter_with_apiKey_fails_invalid_issued_at_claim(){
        ...
        assertThat(response, containsString("Create FAILED: [mntner] SSO-MNT"));
        assertThat(response, containsString("***Warning: [APIKEY] An error occurred while attempting to decode the Jwt: Token\n" +
                "            issued in future"));
    }
    ...
    public void create_mntner_only_data_parameter_with_apiKey_fails_expired_session(){
        ...
        assertThat(response, containsString("Create FAILED: [mntner] SSO-MNT"));
        assertThat(response, containsString("***Warning: [APIKEY] Token has expired"));
    }

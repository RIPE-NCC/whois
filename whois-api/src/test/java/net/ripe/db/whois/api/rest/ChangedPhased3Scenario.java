...
// @formatter:off
    ...
    // old mode
            ...
            given( OLD_MODE, OBJ_EXISTS_WITH_CHANGED ).when( REST,    SEARCH                ).then( SUCCESS, OBJ_EXISTS_WITH_CHANGED),
            given( OLD_MODE, OBJ_EXISTS_NO_CHANGED   ).when( REST,    SEARCH                ).then( SUCCESS, OBJ_EXISTS_NO_CHANGED),

            given( OLD_MODE, OBJ_NO_EXISTS           ).when( REST,    META                  ).then( SUCCESS, OBJ_EXISTS_WITH_CHANGED),

            // TELNET
            given( OLD_MODE, OBJ_EXISTS_WITH_CHANGED ).when( TELNET,  SEARCH                ).then( SUCCESS, OBJ_EXISTS_WITH_CHANGED),
            given( OLD_MODE, OBJ_EXISTS_NO_CHANGED   ).when( TELNET,  SEARCH                ).then( SUCCESS, OBJ_EXISTS_NO_CHANGED),

            given( OLD_MODE, OBJ_NO_EXISTS           ).when( TELNET,  META                  ).then( SUCCESS, OBJ_EXISTS_WITH_CHANGED),
    ...
    // new mode
            ...
            given( NEW_MODE, OBJ_EXISTS_WITH_CHANGED ).when( REST,    SEARCH                ).then( SUCCESS, OBJ_EXISTS_NO_CHANGED),
            given( NEW_MODE, OBJ_EXISTS_NO_CHANGED   ).when( REST,    SEARCH                ).then( SUCCESS, OBJ_EXISTS_NO_CHANGED),

            given( NEW_MODE, OBJ_NO_EXISTS           ).when( REST,    META                  ).then( SUCCESS, OBJ_NO_EXISTS),

            // TELNET
            given( NEW_MODE, OBJ_EXISTS_WITH_CHANGED ).when( TELNET,  SEARCH                ).then( SUCCESS, OBJ_EXISTS_NO_CHANGED),
            given( NEW_MODE, OBJ_EXISTS_NO_CHANGED   ).when( TELNET,  SEARCH                ).then( SUCCESS, OBJ_EXISTS_NO_CHANGED),

            given( NEW_MODE, OBJ_NO_EXISTS           ).when( TELNET,  META                  ).then( SUCCESS, OBJ_NO_EXISTS),
...
// @formatter:on
    ...
    static class Builder {
        ...
        public ChangedPhased3Scenario then(final Result result) {

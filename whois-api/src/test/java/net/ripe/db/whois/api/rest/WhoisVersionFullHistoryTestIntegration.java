...
public class WhoisVersionFullHistoryTestIntegration extends AbstractIntegrationTest {
    ...
    public static final RpslObject AUTNUM_V2 = RpslObject.parse(
            """
            aut-num:        AS102
            as-name:        End-User-2
            descr:          description
    ...
    public static final RpslObject AUTNUM_V3 = RpslObject.parse(
            """
            aut-num:        AS102
            as-name:        End-User-3
            descr:          description
    ...
    @Test
    public void external_user_gets_not_found_for_never_existing_object() {
        assertThrows(NotFoundException.class, () ->
                external("whois/test/aut-num/AS999/versions").get(WhoisResources.class));
    }

    @Test
    public void internal_user_gets_not_found_for_never_existing_object() {
        assertThrows(NotFoundException.class, () ->
                internal("whois/test/aut-num/AS999/versions").get(WhoisResources.class));
    }

    @Test
    public void external_user_gets_not_found_for_single_version_of_deleted_object() {
        databaseHelper.addObject(AUTNUM_V2);
        databaseHelper.deleteObject(AUTNUM_V2);

        assertThrows(NotFoundException.class, () ->
                external(VERSIONS_PATH + "/1", MediaType.TEXT_PLAIN).get(String.class));
    }

    @Test
    public void internal_user_reads_single_version_of_deleted_object() {
        databaseHelper.addObject(AUTNUM_V2);
        databaseHelper.deleteObject(AUTNUM_V2);

        final String response = internal(VERSIONS_PATH + "/1", MediaType.TEXT_PLAIN).get(String.class);

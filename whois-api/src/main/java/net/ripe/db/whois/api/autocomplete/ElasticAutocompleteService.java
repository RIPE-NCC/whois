 ...
 */
@Component
@Path("/elastic/autocomplete")
public class ElasticAutocompleteService {

    private static final Logger LOGGER = LoggerFactory.getLogger(ElasticAutocompleteService.class);

    private static final int MINIMUM_PREFIX_LENGTH = 2;

    private final ElasticAutocompleteSearch elasticAutocompleteSearch;

    @Autowired
    public ElasticAutocompleteService(final ElasticAutocompleteSearch elasticAutocompleteSearch) {
        this.elasticAutocompleteSearch = elasticAutocompleteSearch;
    }
    ...
    public Response lookup(
        ...
        try {
            if (!Strings.isNullOrEmpty(query) && !Strings.isNullOrEmpty(field)) {
                ...
                return okResponse(elasticAutocompleteSearch.search(query, getLookupAttributes(field), getAttributeTypes(attributes), Collections.emptySet()));
            } else if (!select.isEmpty() && !where.isEmpty() && !Strings.isNullOrEmpty(like)) {

                // query by attribute(s)

                return okResponse(elasticAutocompleteSearch.search(like, getAttributeTypes(where), getAttributeTypes(select), getObjectTypes(from)));
            } else {

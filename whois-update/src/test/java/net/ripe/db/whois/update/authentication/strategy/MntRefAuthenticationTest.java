...
public class MntRefAuthenticationTest {
    ...
    @InjectMocks
    private MntRefAuthentication subject;

    @Test
    public void supports_update_with_new_mnt_references() {
        when(update.getNewValues(AttributeType.MBRS_BY_REF)).thenReturn(ciSet("MNT2"));

        assertThat(subject.supports(update), is(true));
    }

    @Test
    public void no_difference_in_mnt_refs_is_not_supported() {
    ...
    public void authentication_succeeds() {
        ...
        final RpslObject mnt = RpslObject.parse("mntner: REF1-MNT\nmnt-ref: REF-MNT");
        final List<RpslObject> mnts = Lists.newArrayList(mnt);
        when(rpslObjectDao.getByKeys(eq(ObjectType.MNTNER), anyCollection())).thenReturn(mnts);
        ...
        when(credentialValidators.authenticate(eq(update), eq(updateContext), anyList(), eq(MntRefAuthentication.class))).thenReturn(candidates);
    ...
    public void no_mntnerref_found_not_error_thrown() {
        ...
        final List<RpslObject> mnts = Lists.newArrayList(RpslObject.parse("mntner: REF1-MNT"));
        when(rpslObjectDao.getByKeys(eq(ObjectType.MNTNER), anyCollection())).thenReturn((mnts));

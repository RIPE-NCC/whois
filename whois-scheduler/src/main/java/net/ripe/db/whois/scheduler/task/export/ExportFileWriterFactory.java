...
class ExportFileWriterFactory {
    ...
    public List<ExportFileWriter> createExportFileWriters(final File baseDir, final int lastSerial) {
        ...
        try {
            FileCopyUtils.copy(String.valueOf(lastSerial).getBytes(StandardCharsets.UTF_8), new File(fullDir,
                    String.format("%s.%s", source, CURRENTSERIAL_SUFFIX)));
            FileCopyUtils.copy(String.valueOf(lastSerial).getBytes(StandardCharsets.UTF_8), new File(fullDir,
                    String.format("%s.%s", nonAuthSource, CURRENTSERIAL_SUFFIX)));
        } catch (IOException e) {
